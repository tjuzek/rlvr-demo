"""
RLVR training: fine-tune a small instruct model on GSM8K math with GRPO.

Defaults target Gemma-2-2B-IT with G=8 generations, temperature 1.0,
learning rate 2e-5, KL coeff 0.005, and 400 training steps. This is
the "update-budget unlocked" recipe: same model and reward variance
setup as ../gemma-rlvr/, but with learning rate 4x higher, KL coeff
10x lower, and 2x more steps. The prior run logged KL max 0.0011
across 200 steps — the policy barely moved, so there was no way to
tell whether the reward signal was useful. This recipe aims to let
the policy actually drift.

The pipeline:
  1. Load the model with LoRA + 4-bit quantization.
  2. For each prompt, generate N completions at the configured temperature.
  3. Score each with the math verifier (final-answer exact-match = 1, else 0).
  4. Update the policy using GRPO — advantage is reward minus group mean.
...
Usage:
    ...
    # Override recipe knobs (logged to training_config.json)
    python train.py --num-generations 8 --temperature 1.0
...
def main():
    ...
    parser.add_argument("--num-generations", type=int, default=NUM_GENERATIONS,
                        help="G: completions per prompt in each GRPO group")
    ...
    num_generations = 2 if args.quick else args.num_generations
