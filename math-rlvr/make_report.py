"""
Post-run report generator for the RLVR-on-GSM8K math pipeline.

Adapted from code-rlvr/make_report.py with minor framing changes for the
post-talk update (header, chart titles, intro callout). Core charting and
styling logic is shared.
...
Writes:
  - results/gsm8k_update.html  (standalone, dark-theme Plotly report)
...
<div class="sibling-note" style="border-left-color: {WARN};">
  <b>About this update.</b>
  This page follows up on the SC-AI Seminar talk on RLVR. In the demo during the
  talk we showed a code-generation run (MBPP) whose pre/post gains were close to
  zero because OLMo-2-7B-Instruct is not code-tuned. This page shows the same
  recipe rerun on <b>GSM8K math</b>, where the base model has real signal —
  closer to the setting the Tulu 3 paper targets.
  <a href="../code-rlvr/results/grpo_report.html">See the original code-rlvr report →</a>
</div>"""
...
<title>RLVR on GSM8K — Post-Talk Update</title>
...
<header class="report-header">
  <div>
    <div class="eyebrow">Post-Talk Update · RLVR · GSM8K</div>
    <h1>RLVR on GSM8K — Post-Talk Update{f" · {escape(label)}" if label else ""}</h1>
...
<footer>
  Generated by math-rlvr/make_report.py · source:
...
def main():
    ...
    parser.add_argument("--out", type=Path,
                        default=ROOT / "results" / "gsm8k_update.html")
